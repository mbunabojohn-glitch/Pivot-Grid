...
const { sendWithdrawalInstruction } = require('../services/WithdrawalService');
...
router.get('/summary', requireAuth, async (req, res) => {
  try {
    ...
    const receipt = await require('../models/CommissionReceipt').findOne({ accountId, splitRef: split._id }).sort({ settledAt: -1 }).lean();
    return res.json({
