...
const Dashboard = () => { 
  ...
  const latestEquity = state.equity.length ? state.equity[state.equity.length - 1].equity : null;
  ...
  return ( 
    <Layout> 
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4"> 
        <div className="bg-slate-900 text-gray-100 p-4 sm:p-5 md:p-6 rounded-xl shadow min-h-24"> 
          <h2 className="text-gray-100">Account Balance</h2> 
          <p className="text-2xl font-bold mt-2 text-gray-100">
            {state.connection.status === 'connected' ? `$${fmt(state.balance)}` : loading ? "Loading..." : `$${fmt(userData?.balance)}`}
          </p> 
        </div> 
        <div className="bg-slate-900 text-gray-100 p-4 sm:p-5 md:p-6 rounded-xl shadow min-h-24"> 
        ...
        <div className="bg-slate-900 text-gray-100 p-4 sm:p-5 md:p-6 rounded-xl shadow min-h-24"> 
          ...
          <p
            className={`text-2xl font-bold mt-2 ${
              state.connection.status === 'connected'
                ? (state.weekly?.netPnL ?? 0) >= 0 ? "text-green-400" : "text-red-400"
                : loading
                  ? ""
                  : (userData?.profit ?? 0) >= 0
                    ? "text-green-400"
          ...
          >
            {state.connection.status === 'connected'
              ? `${(state.weekly?.netPnL ?? 0) >= 0 ? "+" : "-"}$${fmt(Math.abs(state.weekly?.netPnL ?? 0))}`
              : loading
                ? "Loading..."
      ...
      <div className="mt-8 sm:mt-10 bg-slate-900 text-gray-100 p-4 sm:p-5 md:p-6 rounded-xl shadow"> 
        <h2 className="text-lg sm:text-xl font-semibold mb-3 sm:mb-4 text-gray-100">PnL Chart</h2>
        <PnlChart equity={state.equity} />
      </div>
      <div className="mt-6 bg-slate-900 text-gray-100 p-4 sm:p-5 md:p-6 rounded-xl shadow"> 
